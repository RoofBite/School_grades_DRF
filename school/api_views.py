...
from .serializers import (
    ...
    StudentSerializerForList,
    TeacherSerializer,
    TeacherSerializerForTeachersList,
    SchoolClassSerializerForList,
    StudentSerializerGrades,
    StudentsInSubjectSerializerForList,
    PostSerializer,
)
...
from .permissions import (
    ...
    SubjectTeacherGradePermission,
    StudentGradePermission,
    SchoolPostsTeacherOrPrincipalPermission,
    SchoolPostDetailAuthor,
    NotAllowed,
)
...
class SchoolPostDetail(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        try:
            obj = Post.objects.select_related("author", "school").get(
                school__id=pk1, id=pk2
            )
...
class ListSchoolTeachers(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .select_related("user", "schoolclass")
        )
...
class ListSchoolStudents(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Student.objects.filter(school__id=pk)
...
class ListSubjectStudents(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Student.objects.filter(subject__id=pk).select_related(
            "school", "school_class"
        )
...
class StudentGradeInSubject(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return Grade.objects.select_related("student", "subject").filter(
            subject__id=pk1, student__user__id=pk2
        )
...
class StudentGradeInSubjectDetail(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        try:
            return Grade.objects.select_related("student", "subject").get(
                id=pk3, subject__id=pk1, student__user__id=pk2
            )
...
class StudentInSubjectDetail(generics.RetrieveAPIView):
    ...
    def get_object(self):
        ...
        return Student.objects.select_related("school_class").get(
            subject__id=pk1, user__id=pk2
        )
...
class ListSchoolClasses(generics.ListAPIView):
    ...
    def get_queryset(self):
        pk = self.kwargs["pk"]
        return (
            SchoolClass.objects.filter(school__id=pk)
            .select_related("supervising_teacher")
            .prefetch_related("subject")
        )
